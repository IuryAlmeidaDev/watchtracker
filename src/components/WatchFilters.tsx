...
export function WatchFilters({query, selected, onQuery, onSelected, count, customTags=[]}: {
  query: string; selected: string[]; onQuery: (query: string) => void; onSelected: (tags: string[]) => void; count: number; customTags?: string[];
}) {
  const id = useId();
  return <section className="watch-filters" aria-label="Filtros de relógios">
    ...
    <details className="filter-disclosure"><summary><SlidersHorizontal size={16}/>Filtrar características {selected.length > 0 && `(${selected.length})`}</summary>
      ...
      <div className="filter-groups">{Object.entries({...tagGroups,...(customTags.length?{'Outras características':customTags}:{})}).map(([group,tags]) => <fieldset key={group}><legend>{group}</legend><div className="tag-options">{tags.map(tag => <label key={tag} className="tag-option"><Checkbox checked={selected.includes(tag)} onCheckedChange={checked => onSelected(checked ? [...selected,tag] : selected.filter(item=>item!==tag))}/><span>{tag}</span></label>)}</div></fieldset>)}</div>
    </details>
